...
namespace Dowsify.Main.DsUtils
{
    ...
    {
        private static readonly string OverlayTablePath = RomData.OverlayTablePath;
        private static readonly string WorkingDirectory = RomData.WorkingDirectory;
        ...
        public static bool CheckOverlayHasCompressionFlag(int overlayNumber)
        {
            ...
            try
            {
                using FileStream fileStream = File.Open(OverlayTablePath, FileMode.Open, FileAccess.Read, FileShare.Read);
        ...
        {
            using var fileStream = new FileStream(OverlayTablePath, FileMode.Open, FileAccess.Read, FileShare.Read, bufferSize: 4096, useAsync: true);
        ...
        public static string OverlayFilePath(int overlayNumber) => $"{WorkingDirectory}overlay\\overlay_{overlayNumber:D4}.bin";
        ...
        public static void SetOverlayCompressionInTable(int overlayNumber, byte compression)
        {
            ...
            try
            {
                using FileStream fs = new(OverlayTablePath, FileMode.Open, FileAccess.Write);
